...
# multiplier for each letter in the line if they all match
symbol_value = {"A": 5, "B": 4, "C": 3, "D": 2}


def check_winnings(columns, lines, bet, values):
    winnings = 0
    # this will tell the user which line(s) they won on
    winning_lines = []
    ...
    for line in range(lines):
        ...
        for column in columns:
            ...
            if symbol != symbol_to_check:
                break
            # if we didnt break out of the loop meaning that the symbols matched, then go to else statement and add the values multiplier * the bet that the user bet on on that line
        else:
